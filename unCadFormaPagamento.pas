...
begin
  ...
  if MessageBox(fmPrincipal.Handle,
                PWideChar('Confirma a exclusão da forma de pagamento?'),
...
begin
  ...
  if AnsiCompareText(EditCadastroNMFORMAPAGAMENTO.Text,EmptyStr) = 0 then
    vMensagem := vMensagem + #13#10'- O campo Forma de Pagamento é requerido.';
    ...
    try
      ...
      begin
        ...
        begin
          ...
          raise exception.Create('O cadastro da Forma de Pagamento não foi encontrado.');
      end;
    finally
      vQuery.Active := false;
      FreeAndNil(vQuery);
...
begin
  try
    ...
    Ordenacao := '|"Código":"N":"Asc."|;|"Forma de pagamento":"S":"Asc."|;|"Ativo":"S":"Asc."|';
...
begin
  try
    try
      ...
      // Configura a Grid.
      editaColunaGrid(1,'CDFORMAPAGAMENTO','Código',edNone,taRightJustify,40,true);
...
begin
  ...
  try
    ...
    if pFormaPagamento.existeDependencia then
      vMensagem := '- Existem aquisições ou parcelas de aquisições relacionadas a esta forma de pagamento.';
