...
begin
  ...
  if MessageBox(fmPrincipal.Handle,
                PWideChar('Confirma a exclusão do motivo de urgência?'),
...
begin
  ...
  if AnsiCompareText(EditCadastroDEMOTIVOURGENCIA.Text,EmptyStr) = 0 then
    vMensagem := vMensagem + #13#10'- O campo Motivo de Urgência é requerido.';
...
begin
  ...
  try
    try
      ...
      with vQuery do
      begin
        // Consulta o cadastro do Motivo de Urgência.
        ...
        begin
          ...
          raise exception.Create('O cadastro do Motivo de Urgência não foi encontrado.');
      end;
    finally
      vQuery.Active := false;
      FreeAndNil(vQuery);
...
begin
  try
    ...
    Ordenacao := '|"Código":"N":"Asc."|;|"Motivo de urgência":"S":"Asc."|;|"Ativo":"S":"Asc."|';
...
begin
  try
    try
      ...
      // Configura a Grid.
      editaColunaGrid(1,'CDMOTIVOURGENCIA','Código',edNone,taRightJustify,40,true);
      editaColunaGrid(2,'DEMOTIVOURGENCIA','Motivo de urgência',edNone,taLeftJustify,450,true);
...
begin
  ...
  try
    ...
    if pMotivoUrgencia.existeDependencia then
      vMensagem := '- Existem aquisições relacionadas a este motivo de urgência.';
